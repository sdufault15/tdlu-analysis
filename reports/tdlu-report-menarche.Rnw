...
# p.val.age <- age_mod %>% filter(term == "Age") %>% select(p.value)
# p.val.bmi <- bmi_mod %>% filter(term == "BMI") %>% select(p.value)
# p.val.menarche <- menarche_mod %>% filter(term == "Menarche") %>% select(p.value)
# p.val.afb <- afb_mod %>% filter(term == "AFB") %>% select(p.value)
# p.val.parity <- parity_mod$p.value
# p.val.famhx <- familyhx_mod$p.value
@
...
it’s usually taken as zero because this is most often meaningful in the context
...
data.frame(t.alleles = as.factor(dta$`T alleles`), pih = dta$PIH, pred.tdlu = predict(m_nb_adj_int, type = "response")) %>%
 ...
 # ggplot(aes(x = `No. T Alleles`, y = pred.tdlu, col = pih, fill = pih)) +  # 002
  ...
  labs(caption = #paste0("Boxplots denote the adjusted TDLU count IQR with horizontal median line.\nWhiskers span the empirical range of points between ", unname(TeX("$Q_1")), " - 1.5*IQR), ", expression(Q[3] + 1.5*IQR), "]")) + 
  unname(TeX("Boxes span the adjusted TDLU count interquartile range (IQR). Whiskers span empirical range within ($Q_1-1.5*IQR, Q_3+1.5*IQR) ."))) +
...
\begin{table}[!h]
\begin{threeparttable}
\caption{Summary table of the adjusted T allele IRRs from the models stratified on HDP-status.}
\begin{tabular}{@{}rcccccc@{}}
\toprule
 &\phantom{abcd}& \multicolumn{2}{c}{\textbf{HDP-Negative}} &\phantom{abcde}& \multicolumn{2}{c}{\textbf{HDP-Positive}} \\
 \cmidrule{3-4} \cmidrule{6-7}
 && IRR (95\% CI)  &  p-value & & IRR (95\% CI) &  p-value  \\ %\multicolumn{1}{@{}l}{\textbf{HDP-Negative} \hspace{2em}}  \multicolumn{1}{l}{\textbf{HDP-Positive}}
\midrule
...
p5 <- dta %>%
  ...
  geom_line(aes(x = Parity, y = TDLU.pred),
            ...
            lwd = 1, 
            lty = 1) + 
  #stat_summary(fun.y = mean, geom = "line", aes(group = 1), lty = 3, col = swatch()[5]) + 
  #stat_summary(fun.y = mean, geom = "point", pch = 18, size = 3, col = swatch()[5]) + 
  ...
  # stat_summary(fun.y = median, geom = "point", pch = 20, size = 4, col = swatch()[4]) + 
  # stat_summary(fun.y = median, geom = "line", aes(group = 1), lty = 3, col = swatch()[4]) + 
  #stat_summary(fun.data = "mean_cl_boot", geom = "errorbar", size = 0.2, col = swatch()[3], width = .1) + 
  xlab("Parity") + 
  ylab("TDLU Counts")
p5
...
%For the observed distribution the histogram is drawn on a square root scale (hence the name) and superimposed with a line for the fitted frequencies.

<<rootogram_exp-counts-zi, echo = FALSE, fig.cap = "Rootogram. The bars represent the observed distribution of TDLU counts while the red line represents the model-fitted values. We see that we are overpredicting 0s, underpredicting 1s, overpredicting 2-4s, overpredicting 5s, etc.">>=
rootogram(m_nb,
          style = "hanging",
          scale = "raw",
          main = "Negative Binomial Model\nExpected versus Observed Counts",
          xlab = "Observed TDLUs",
          sub = "Fitted Values in Red",
          cex.sub = 0.7)
@
